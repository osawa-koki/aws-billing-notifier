...
```bash
aws-billing-notifier$ sam build
```
...
```bash
aws-billing-notifier$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
aws-billing-notifier$ sam local start-api
aws-billing-notifier$ curl http://localhost:3000/
```
...
```bash
aws-billing-notifier$ sam logs -n HelloWorldFunction --stack-name aws-billing-notifier --tail
```
...
```bash
aws-billing-notifier$ cd hello-world
...
```bash
sam delete --stack-name aws-billing-notifier
```
